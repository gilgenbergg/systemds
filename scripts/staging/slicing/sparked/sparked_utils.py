...
def make_first_level(features, predictions, f_l2, top_k, alpha, k, w, loss_type):
    first_level = []
    ...
    for feature in features:
        ...
        new_node.score = opt_fun(new_node.loss, new_node.size, f_l2, len(predictions), w)
...
def process_node(node_i, level, f_l2, predictions, cur_lvl, top_k, alpha, loss_type, w, debug, enumerator):
    cur_enum_nodes = []
    for node_j in level:
        if enumerator == "join":
...
def nodes_enum(nodes, level, predictions, f_l2, top_k, alpha, k, w, loss_type, cur_lvl, debug, enumerator):
    cur_enum_nodes = []
    for node_i in nodes:
        partial_nodes = process_node(node_i, level, f_l2, predictions, cur_lvl, top_k, alpha,
