...
def parallel_process(all_features, predictions, f_l2, sc, debug, alpha, k, w, loss_type, enumerator):
    top_k = Topk(k)
    cur_lvl = 0
    levels = []
    ...
    while len(levels) > 1:
        ...
        mapped = part.map(lambda nodes: sparked_utils.nodes_enum(nodes, levels, predictions, f_l2,
        ...
        print("Level " + str(cur_lvl) + " had " + str(len(levels) * (len(levels) - 1)) +
              " candidates but after pruning only " + str(len(lvl_nodes_res)) + " go to the next level")
